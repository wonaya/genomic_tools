...
plot.mixmdl=function(dir, comp, main,d){
  flt.cpgs=d[d$pvalue <= 0.05,]
  myMixmdl=myDiff.to.mixmdl(flt.cpgs[flt.cpgs$meth.diff>0,])
  pdf(paste(dir, comp, ".hyper.mixmdl.pdf", sep=""))
  plotMdl1(myMixmdl, subtitle=paste(main, "hyper methylation"), cex.sub=1.2)
  dev.off()
  write.table(2^get.break_point(myMixmdl), col.names=F, row.names=F, file=paste(dir, comp, ".hyper.mixmdl.txt"))
  pdf(paste(dir, comp, ".hypo.mixmdl.pdf", sep=""))
  myMixmdl=myDiff.to.mixmdl(flt.cpgs[flt.cpgs$meth.diff<0,])
  plotMdl1(myMixmdl, subtitle=paste(main, "hypo methylation"), cex.sub=1.2)
  dev.off()
  write.table(2^get.break_point(myMixmdl), col.names=F, row.names=F, file=paste(dir, comp, ".hypo.mixmdl.txt"))
}

stouffer=function(x)pnorm(sum(qnorm(x))/sqrt(length(x))) 

SLcombine=function(pvals,sigma) pnorm(sum(solve(chol(sigma, pivot=T))%*%qnorm(pvals))/sqrt(length(pvals)))
...
getDMR2=function(peaks, allMyDiff, pcutoff=0.1,step=100, DMC.qvalue=0.01, DMC.methdiff=25, num.DMCs=1, num.CpGs=3, DMR.methdiff=20){
  library(GenomicRanges,quietly =TRUE)
  library(data.table,quietly =TRUE)
  myDiff=allMyDiff[allMyDiff$ppvalue<=pcutoff,]
  myDiff.gr=GRanges(seqnames=Rle(myDiff$pchr), IRanges(start=as.integer(myDiff$pstart), end=as.integer(myDiff$pend)), strand=myDiff$pstrand, pval=myDiff$ppvalue, padj=myDiff$pqvalue, methDiff=myDiff$pmethdiff)
...
dist_to_mixmdl <- function(dist)
{
  ...
  mixmdl=normalmixEM(log2.distance, lambda = NULL, mu = NULL, sigma = NULL, k = 2, mean.constr = NULL, sd.constr = NULL,epsilon = 1e-08, maxit = 1000, maxrestarts=20, verb = FALSE, fast=FALSE, ECM = FALSE,arbmean = TRUE, arbvar = TRUE)
}
...
myDiffToDMR=function(myDiff, dist=100, step=100, DMC.qvalue=0.01, DMC.methdiff=25, num.DMCs=1, num.CpGs=3, DMR.methdiff=20, ACF=TRUE){
  ...
  if(ACF==TRUE){
    res=getDMR(peaks, myDiff.new, pcutoff=1, step=step, DMC.qvalue=DMC.qvalue, DMC.methdiff=DMC.methdiff, num.DMCs=num.DMCs, num.CpGs=num.CpGs, DMR.methdiff=DMR.methdiff)  
  } else if (ACF==FALSE){
...
# main eDMR function
# myDiff 
eDMR.sub=function(myDiff, step=100, dist="none", DMC.qvalue=0.01, DMC.methdiff=25, num.DMCs=1, num.CpGs=3, DMR.methdiff=20, granges=TRUE, plot=FALSE, main="", direction="both", ACF=TRUE){
  if(direction=="both"){
  ...
  DMR=myDiffToDMR(myDiff, dist=dist, step=step, DMC.qvalue=DMC.qvalue, DMC.methdiff=DMC.methdiff, num.DMCs=num.DMCs, num.CpGs=num.CpGs, DMR.methdiff=DMR.methdiff, ACF=ACF)
