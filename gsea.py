...
outfile4 = open(hg_outfile, 'w')
for e in open(hg_file+".txt", 'r') :
    # GO GO-desc FisherPvalue count (gene in GO ,population, overlap,gene set) GeneMerge pval (HG)
    robjects.r("cont.table = matrix(nrow=2, ncol=2)")
    robjects.r('cont.table[1,1] = '+str(int(e.split("\t")[2])-int(e.split("\t")[4])))
    robjects.r('cont.table[2,1] = '+str(int(e.split("\t")[3])-int(e.split("\t")[5])-(int(e.split("\t")[2])-int(e.split("\t")[4]))))
    robjects.r('cont.table[1,2] = '+str(int(e.split("\t")[5])-int(e.split("\t")[4])))
    robjects.r('cont.table[2,2] = '+str(int(e.split("\t")[4])))
    robjects.r('pval <- fisher.test(cont.table, alternative="two.sided")$p.value')
    robjects.r('padj <- p.adjust(pval, method="bonferroni", n=1684)')
    pval = robjects.globalenv['pval']
    padj = robjects.globalenv['padj']
    outfile4.write(e.split("\t")[0])
    outfile4.write("\t")
    outfile4.write(e.split("\t")[-1].strip("\n"))
    outfile4.write("\t")
    outfile4.write(str(float(str(pval).split("] ")[1].strip("\n"))))
    outfile4.write("\t")
    outfile4.write(str(float(str(padj).split("] ")[1].strip("\n"))))
    outfile4.write("\t")
    outfile4.write("\t".join(e.split("\t")[2:6]))
    outfile4.write("\t")
    outfile4.write(e.split("\t")[6])
    outfile4.write("\n")
outfile4.close()
sys.exit()
