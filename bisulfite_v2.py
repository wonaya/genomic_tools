...
def max_coord(name, chr) :
    ...
    print "max value get"
    max_val_cpg = []
    for a in open("CpG_context_"+str(name)+"_chr"+str(chr)+"_sorted.out", 'r') :
        max_val_cpg.append(int(a.split("\t")[3]))
    last.append(max(max_val_cpg))
    del max_val_cpg
    max_val_chg = []
    for a in open("CHG_context_"+str(name)+"_chr"+str(chr)+"_sorted.out", 'r') :
        max_val_chg.append(int(a.split("\t")[3]))
    last.append(max(max_val_chg))
    del max_val_chg
    max_val_chh = []
    for a in open("CHH_context_"+str(name)+"_chr"+str(chr)+"_sorted.out", 'r') :
       max_val_chh.append(int(a.split("\t")[3]))
    last.append(max(max_val_chh))
    del max_val_chh
...
def tile(name, chr, index) :#sort methylation extracted file by coordinates
    ...
    print "tile : reading CpG files", index, datetime.datetime.now()
    ...
    for cpgline in open("CpG_context_"+str(name)+"_chr"+str(chr)+"_sorted.out", 'r') :
        if int(cpgline.split("\t")[3]) >= x*1000000 and int(cpgline.split("\t")[3]) < (x+1)*1000000 :
            cpg_split_lines.append(cpgline.split("\t")[1]+"\t"+cpgline.split("\t")[3])
    print "tile : reading CHH files", index, datetime.datetime.now()
    chh_split_lines = []
    for chhline in open("CHH_context_"+str(name)+"_chr"+str(chr)+"_sorted.out", 'r') :
        if int(chhline.split("\t")[3]) >= x*1000000 and int(chhline.split("\t")[3]) < (x+1)*1000000 :
            chh_split_lines.append(chhline.split("\t")[1]+"\t"+chhline.split("\t")[3])
    print "tile : reading CHG files", index, datetime.datetime.now()
    chg_split_lines = []
    for chgline in open("CHG_context_"+str(name)+"_chr"+str(chr)+"_sorted.out", 'r') :
        if int(chgline.split("\t")[3]) >= x*1000000 and int(chgline.split("\t")[3]) < (x+1)*1000000 :
            chg_split_lines.append(chgline.split("\t")[1]+"\t"+chgline.split("\t")[3])
    print "finished reading", index, datetime.datetime.now()
    ...
    for y in range(x*10000, (x+1)*10000) :
        ...
        for cpg_split_line in cpg_split_lines :
            if int(cpg_split_line.split("\t")[1]) >= y*100 and int(cpg_split_line.split("\t")[1]) < (y+1)*100 :
                if cpg_split_line.split("\t")[0] == "+" :
                    metCpG += 1
                    totCpG += 1
                elif cpg_split_line.split("\t")[0] == "-" :
                    totCpG += 1
        metCHH = 0 
        totCHH = 0
        for chh_split_line in chh_split_lines :
            if int(chh_split_line.split("\t")[1]) >= y*100 and int(chh_split_line.split("\t")[1]) < (y+1)*100 :
                if chh_split_line.split("\t")[0] == "+" :
                    metCHH += 1
                    totCHH += 1
                elif chh_split_line.split("\t")[0] == "-" :
                    totCHH += 1
    
        metCHG = 0 
        totCHG = 0
        for chg_split_line in chg_split_lines :
            if int(chg_split_line.split("\t")[1]) >= y*100 and int(chg_split_line.split("\t")[1]) < (y+1)*100 :
                if chg_split_line.split("\t")[0] == "+" :
                    metCHG += 1
                    totCHG += 1
                elif chg_split_line.split("\t")[0] == "-" :
                    totCHG += 1
    ...
    outfile.close()
    print "finished writing", index, datetime.datetime.now()
...
if __name__ == '__main__':
    ...
    parser.add_option("--index", default=None, help="given numcore is 10, if 0, go from 0 to 9, if 1, go from 10 to 19 etc..")
    parser.add_option("--maxval", default=None, help="max coordinate / 1M + 1")
    ...
    if options.run == "bismark" :
        ...
        for name in options.name.split(",") : 
            for chr in options.chrom.split(",") :
                if options.maxval == None :
                    max_index = int(float(max_coord(name, chr))/1000000)+1
                else :
                    max_index = int(options.maxval)
                print "maxval :", max_index
                large_list = []
                small_list = []
                ...
                if len(small_list) > 0 :
                    large_list.append(small_list)
                del small_list
                print "running: ", large_list[int(options.index):int(options.index)+1][0]
                for small_list in large_list[int(options.index):int(options.index)+1] :
                    jobs = []
