...
class tile_class :
    ...
    @staticmethod
    def tile_all_context_paired(name, chr, window) :
...
parser = OptionParser()
parser.add_option("--run", dest="run", help="type of run")
parser.add_option("--sam", dest="samfile", help="name and directory of sam file generated from bismark")
...
if options.run == "test" :
    ...
    for x in range(0, total_rounds) :
        jobs = []
        for chr in setup.get_genome_size(options.genome)[0].keys()[x*12:(x+1)*12] :
            s = multiprocessing.Process(target=tile_class.tile_context_sp_single_write, args=(chr, options.samfile, options.genome, options.context, options.window, ))
            jobs.append(s)
        [x.start() for x in jobs]
        [x.join() for x in jobs]
    tile_class.tile_context_sp_single_merge(options.samfile, options.genome, options.context, options.window)
"""
