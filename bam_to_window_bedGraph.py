...
for a in range(1, max_chr) : 
    for b in range(0, chromosome_size[a]) :
        diff = 10 - len(str(b*int(window)+1))
        ID = '0'*diff
        diff2 = 10 - len(str(b*int(window)+(halfwindow+1)))
        ID2 = '0'*diff2
        outfile.write(str(a)+"\t.\tWindow\t"+str(b*int(window)+1)+"\t"+str((b+1)*int(window))+"\t.\t.\t.\tID="+str(a)+str(ID+str(b*int(window)+1))+"\n")
        outfile.write(str(a)+"\t.\tWindow\t"+str((b+1)*int(window)-(halfwindow-1))+"\t"+str((b+1)*int(window)+halfwindow)+"\t.\t.\t.\tID="+str(a)+str(ID2+str(b*int(window)+(halfwindow+1)))+"\n")
outfile.close()

print "###running samtools index ..."
for bams in list_of_bams :
    if not os.path.isfile(bams+".bai") :
        os.system(str(location_of_samtools)+" index "+str(bams))
